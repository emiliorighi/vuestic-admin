  ...
  <va-inner-loading :loading="isLoading">
    <div class="layout">
      ...
      <div v-if="showNotifications" class="row">
        <div class="flex lg12 md12 sm12 xs12">
          <va-card class="custom-card">
            <va-card-title> Notifications </va-card-title>
            <va-card-content v-for="msg in response.messages" :key="msg">
              <va-alert
                v-for="key in Object.keys(msg)"
                :key="key"
                border="left"
                :title="key"
          ...
          <va-card class="custom-card">
            ...
            <va-card-content>
              <div class="row justify--space-between align--center">
                <div class="flex">
                  <va-file-upload v-model="excel.file" file-types=".xlsx" :disabled="fileLoaded" undo>
            ...
            <va-divider />
            <va-card-content>
              <va-form ref="excelform" tag="form" @validation="validForm = $event" @submit.prevent="submitExcel">
                <va-select
      ...
      <div class="row justify--space-between">
        <div class="flex">
          <va-button color="danger" type="reset" @click="resetForm()"> Reset Form </va-button>
        </div>
        <div class="flex">
          <va-button @click="$refs.excelform.validate()"> Validate Form </va-button>
          <va-button :disabled="!validForm && fileLoaded === 1" type="submit" @click="submitExcel()">
            Submit
          </va-button>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  const formOptions = [
    ...
    {
      label: 'Broker source',
      message: 'lower case name of the broker service. Ex: copo; leave this field empty if data is only local',
      key: 'source',
    },
  ]
